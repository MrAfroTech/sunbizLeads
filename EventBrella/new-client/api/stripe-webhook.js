...
// Templated from FarmerBanks (working reference). Expects same checkout.session.completed
// payload shape: metadata.tier, event_time, event_venue, event_date, ticket_count,
// organizer_name, event_name, customer_name, transaction_id, (optional platform_fee).
...
async function handleCheckoutCompleted(session) {
  ...
  try {
    ...
    const eventName = metadata.event_name || process.env.CLIENT_DEFAULT_EVENT_NAME || '—';
    ...
    const eventVenue = metadata.event_venue || process.env.CLIENT_VENUE_NAME || '—';
...
async function generateQRCode(transactionId, metadata) {
  ...
  const baseUrl = process.env.BASE_URL || 'https://new-client.eventbrella.us';
    ...
    for (let i = 0; i < metadata.ticketCount; i++) {
      ...
      const qrData = {
        ticketId: ticketId,
        transactionId: transactionId,
        eventName: metadata.eventName || process.env.CLIENT_DEFAULT_EVENT_NAME || '—',
        ...
        eventVenue: metadata.eventVenue || process.env.CLIENT_VENUE_NAME || '—',
...
async function sendConfirmationEmail(email, name, data) {
  ...
  if (TEST_MODE) {
    ...
    console.log('📨 [EMAIL LOG] Would trigger Klaviyo event:', {
      ...
      subject: `${process.env.CLIENT_VENUE_NAME || '—'} - ${data.eventName || 'Event'}`,
      customerName: name,
    ...
    const eventData = {
      data: {
        ...
        attributes: {
          ...
          properties: {
            event_name: data.eventName || process.env.CLIENT_DEFAULT_EVENT_NAME || '—',
            ...
            venue_name: data.eventVenue || process.env.CLIENT_VENUE_NAME || '—',
            ...
            event_slug: data.eventSlug || process.env.CLIENT_EVENT_SLUG || 'new-client-default',
  ...
  const testPayload = {
    data: {
      ...
      attributes: {
        ...
        properties: {
          eventSlug: process.env.CLIENT_EVENT_SLUG || "new-client-default",
          eventName: process.env.CLIENT_DEFAULT_EVENT_NAME || "—",
          eventDate: "2026-01-03",
          eventTime: "8:00 AM - 10:00 AM EST",
          eventVenue: process.env.CLIENT_VENUE_NAME || "—",
