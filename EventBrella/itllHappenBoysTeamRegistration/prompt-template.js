...
${(data.additionalClients && data.additionalClients.length) ? `
- **Additional clients** (use where the site supports multiple client names): ${data.additionalClients.map(c => (c.name || '').replace(/"/g, '\\"')).filter(Boolean).join('; ') || '—'}
  - Display names: ${data.additionalClients.map(c => (c.displayName || c.name || '—').replace(/"/g, '\\"')).join('; ')}
` : ''}
...
### Points of contact (additional clients/contacts for this event)
Add these to the client site (e.g. contact section, footer, or config) as needed:
\`\`\`
${data.contacts.map((c, i) => `${i + 1}. ${(c.name || '—').replace(/"/g, '\\"')} | ${(c.email || '—').replace(/"/g, '\\"')}${c.phone ? ' | ' + c.phone.replace(/"/g, '\\"') : ''}${c.role ? ' | ' + c.role.replace(/"/g, '\\"') : ''}`).join('\n')}
\`\`\`
` : ''}
...
window.exampleClientData = {
  ...
  contacts: [
    { name: 'Box Office', email: 'boxoffice@example.com', phone: '(555) 111-2222', role: 'Box office' },
    { name: 'Venue Contact', email: 'venue@example.com', phone: '', role: 'Venue' }
  ],
  additionalClients: [
    { name: 'Second Venue Co', displayName: 'Second Venue (partner)' }
  ]
};
