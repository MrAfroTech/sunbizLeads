...
const JWT_SECRET = process.env.JWT_SECRET || 'orlando-pirates-wallet-secret-key-change-in-production';
  ...
  try {
    ...
    const passServiceResponse = await fetch(`${walletPassServiceUrl}/api/passes/apple`, {
      ...
      body: JSON.stringify({
        ...
        mode,
        qr_token: qrToken,
        user_name: wallet.userName || 'Orlando Pirates Fan'
      })
    });
    ...
    const { data: passRecord, error: passError } = await supabase
      ...
      .insert({
        ...
        pass_type_id: process.env.APPLE_PASS_TYPE_ID || 'pass.com.orlandopirates.wallet',
