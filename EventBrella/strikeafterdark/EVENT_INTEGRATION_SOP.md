# Event Integration SOP: Strike After Dark → EventBrella UI
...
This document outlines the current event structure in the Strike After Dark codebase and provides a roadmap for integrating these events with the EventBrella UI platform. **No code changes have been made** - this is an analysis and integration guide.
...
The Strike After Dark codebase currently defines **3 distinct event types**:
...
- **Table Name:** `StrikeAfterDark-Events-prod` (from env var `EVENTS_TABLE`)
- **Schema Fields:**
...
**Strike After Dark Current Format → EventBrella Format**

| Strike After Dark Field | EventBrella Equivalent | Notes |
|-------------------|----------------------|-------|
