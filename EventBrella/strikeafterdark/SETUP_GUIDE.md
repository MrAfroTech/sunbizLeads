# Strike After Dark Farm Tour - Setup Guide
...
```bash
cd EventBrella/strikeAfterDark
npm install
```
...
2. Add endpoint: `https://your-domain.com/api/strikeAfterDark/stripe-webhook`
3. Select events:
...
1. Ensure `vercel.json` includes routes for strikeAfterDark API
2. Deploy to Vercel
...
```bash
stripe listen --forward-to localhost:3000/api/strikeAfterDark/stripe-webhook
...
2. Add Strike After Dark branding/images
