# Klaviyo Setup Guide for Strike After Dark Ticket System

## Context
You are helping set up Klaviyo for the Strike After Dark Harvest Experience ticket system. The webhook triggers a "Ticket Purchased" event in Klaviyo with all ticket data, and we need to create a Flow to send transactional emails with the ticket QR codes.
...
- `{{ eventVenue }}` - Venue (e.g., "Bosses & Bowling")
- `{{ organizerName }}` - Organizer (e.g., "Strike After Dark")
...
1. Make a test purchase on the Strike After Dark site
...
## Overall Strike After Dark Project Checklist
