/**
 * Scan & Scale site events + phone_capture → scan_and_scale_click_events (funnel INSERT).
 * POST /api/log-site-event
 */
...
module.exports = async function logSiteEvent(req, res) {
  ...
  const row = {
    ...
    campaign:
      body.campaign == null || String(body.campaign).trim() === ''
        ...
        : String(body.campaign).trim().slice(0, 500),
    contact:
      body.contact == null || String(body.contact).trim() === ''
        ? null
        : String(body.contact).trim().slice(0, 500),
    element_label:
  ...
  const { error: insertErr } = await supabase.from('scan_and_scale_site_events').insert(row);
  ...
  if (rawContact && looksLikeEmail(rawContact)) {
    ...
    if (email) {
      ...
      try {
        ...
        // Best-effort; site event already stored.
      }
    }
  }
