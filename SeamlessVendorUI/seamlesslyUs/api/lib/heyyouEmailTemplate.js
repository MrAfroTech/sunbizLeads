...
function generateSportsHeyyouEmail({
  ...
  leftOnTable = 0,
  pdfUrl = '',
  brandLabel = '9 Things · Seamlessly',
  lossContext = 'on the table last game',
  introLine = 'built for operators who want guests spending more without operational gaps between brief and floor.',
}) {
  const firstName = firstNameFromName(name);
  const venueLine = venueName ? ` at <strong>${venueName}</strong>` : '';
  const lossLine =
    leftOnTable > 0
      ? `<p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#444;">
          Based on your calculator run, you left an estimated <strong>${formatMoney(leftOnTable)}</strong> ${lossContext}${venueLine}. The attached guide shows where operators like you close that gap fastest.
        </p>`
      : `<p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#444;">
          The attached guide breaks down the nine operational shifts that move revenue fastest when guest intelligence reaches the venue floor.
        </p>`;
...
<head>
  ...
  <title>Your guide is attached</title>
</head>
        ...
        <table width="100%" style="max-width:560px;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 8px 30px rgba(26,18,8,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,#1a1208 0%,#2d2216 100%);padding:28px 32px;text-align:center;">
              ...
              <p style="margin:10px 0 0;color:#c8a44a;font-size:14px;">${brandLabel}</p>
            </td>
          </tr>
          <tr>
            <td style="padding:32px;">
              ...
              <p style="margin:0 0 24px;font-size:16px;line-height:1.6;">
                Thanks for running your numbers. As promised, here is <strong>Hey You — 9 Things</strong> (PDF attached) — ${introLine}
              </p>
              ${lossLine}
              ${downloadLine}
...
function generateSportsHeyyouPlainText({
  ...
  const lines = [
    `Hi ${firstName},`,
    '',
    'Thanks for running your numbers. As promised, here is Hey You — 9 Things (PDF attached).',
  ];

  if (leftOnTable > 0) {
    lines.push(
      '',
      `Based on your calculator run, you left an estimated ${formatMoney(leftOnTable)} ${lossContext}${
