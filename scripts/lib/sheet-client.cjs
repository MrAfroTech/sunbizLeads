/**
 * Google Sheets client (CommonJS for sunbiz-scraper.cjs).
 * getSunbizDoc()/getSunbizSheet() use SUNBIZ_SPREADSHEET_ID.
 */
const path = require('path');
const envPath = path.resolve(process.cwd(), '.env');
require('dotenv').config({ path: envPath });

const { getGoogleSheetsCredentials } = require('../../config/credentials-config.cjs');
