# Scan & Scale click-event funnel

Automation for `public.scan_and_scale_click_events`: owner alert, Email 1 (with runtime CTA strip), optional call task, and scheduled Emails 2–4 via Brevo templates.
...
```bash
cd eCommerceSite/scan-and-scale/scan-scale-funnel
...
Templates read from `salesMastery/emailSequences/scanAndScaleSequence/scan_scale_email_*.html` by default. Override path with:
...
Working directory must be **`scan-scale-funnel`** so the CLI resolves `scan-scale-funnel/supabase/`.
