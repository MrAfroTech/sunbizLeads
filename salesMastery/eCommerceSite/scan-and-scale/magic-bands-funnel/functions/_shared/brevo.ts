...
/** Runtime strip for MagicBands Email 1 only — removes the BOOK YOUR FREE… CTA <a>; leaves the Deployment Blueprint CTA untouched. */
export function stripBookingCtaFromEmailOneHtml (html: string): string {
  const pattern =
    /<a\s+[^>]*class="cta-btn"[^>]*href="https:\/\/scan-and-scale\.seamlessly\.us\/calculator\/magic-bands"[^>]*>\s*BOOK\s+YOUR\s+FREE\s+REVENUE\s+FIT\s+SESSION\s*→\s*<\/a>/i;
