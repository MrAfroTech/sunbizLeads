-- Include calculator_page_visits in v_lead_priority (newsletter-attributed landings)
-- Fixes silent gap: identity in CPV but absent from lead_events / finished_calc_leads
...
cpv_latest AS (
  SELECT DISTINCT ON (lower(trim(email)))
    ...
    name,
    page_key,
    coalesce(updated_at, created_at) AS cpv_at
  FROM public.calculator_page_visits
  WHERE email IS NOT NULL AND trim(email) <> ''
...
staged AS (
  SELECT
    ...
    NULLIF(trim(coalesce(fl.name, cpv_l.name)), '') AS full_name,
...
COMMENT ON VIEW public.v_lead_priority IS
  'One row per email — funnel stage, priority score, AM status. Includes calculator_page_visits for newsletter-attributed landings.';
