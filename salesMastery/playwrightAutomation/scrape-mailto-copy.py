...
with optional name/title context. Appends results to contacts.csv.
...
CSV_PATH = SCRIPT_DIR / "contacts.csv"
    ...
    *,
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Reset the clock for the button interaction phase so expand time doesn't eat the budget
            button_phase_started = time.monotonic()
            ...
            for i in range(email_button_attempts):
                if (time.monotonic() - button_phase_started) >= 30.0:
                    break
...
def _migrate_csv_add_phone_column(csv_path: Path) -> None:
    """If contacts.csv exists with the old header (no Phone), rewrite with Phone column."""
...
def main():
    ...
    parser.add_argument("--max-seconds", type=float, default=60.0, help="Hard time budget for scraping (default: 60s)")
    parser.add_argument(
        "--max-buttons",
        type=int,
        default=40,
