    ...
    *,
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Navigation can wait longer; max_seconds still caps the scrape phase.
            ...
            try:
                ...
                print("load timeout, scraping whatever loaded")
            try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Read fbsFcsTeams.csv, scrape each school's /staff-directory page, append to contact CSV",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="Process at most this many teams (0 = all rows)",
    )
    ...
    if not input_teams_csv.is_file():
        print(f"Missing team list: {input_teams_csv}", file=sys.stderr)
        sys.exit(1)

    teams: list[dict] = []
    with open(input_teams_csv, newline="", encoding="utf-8") as f:
        teams = list(csv.DictReader(f))

    if args.limit and int(args.limit) > 0:
        teams = teams[: int(args.limit)]

    def write_flagged(tier: str, league: str, team_name: str, website: str, reason: str) -> None:
        exists = flagged_csv.exists()
        with open(flagged_csv, "a", newline="", encoding="utf-8") as fh:
            w = csv.DictWriter(fh, fieldnames=flagged_headers)
            if not exists:
                w.writeheader()
            w.writerow({
                "tier": tier,
    ...
    for i, row in enumerate(teams):
        tier = (row.get("league_tier") or row.get("tier") or "").strip()
        league = (row.get("league") or "").strip()
        team_name = (row.get("team_name") or "").strip()
        website = (row.get("website") or "").strip()
        company = team_name
        base = website.rstrip("/")
        staff_url = f"{base}/staff-directory" if base else ""

        if not staff_url:
            write_flagged(tier, league, team_name, website, "error")
            print(f"[skip] {team_name or '(unknown)'}: no website URL")
            continue

        url = staff_url
        print(f"[scrape] {team_name}: {staff_url}")
        if i < 3:
            print(url)
        try:
            ...
            reason = "timeout" if type(e).__name__ == "TimeoutError" else "error"
            write_flagged(tier, league, team_name, website, reason)
            print(f"  -> flagged ({reason}): {e}")
            continue
        ...
        if not rows:
            write_flagged(tier, league, team_name, website, "no_emails")
            print("  -> flagged: no emails (or load continued with empty result)")
            continue

        append_to_csv(rows, company, CSV_PATH)
        print(f"  -> wrote {len(rows)} row(s) to {CSV_PATH}")

    print(f"Done. Output: {CSV_PATH}; flagged: {flagged_csv}")
