-- Dedupe baseball_teams by normalized team name (matches app: trim, lower, collapse spaces).
...
-- Normalized key (same idea as normalizeTeamName in baseball-tracker.jsx)
...
UPDATE baseball_team_contacts c
...
WITH agg AS (
  ...
  JOIN baseball_teams bt ON bt.id = n.id
  GROUP BY tk.keep_id
)
UPDATE baseball_teams t
SET
