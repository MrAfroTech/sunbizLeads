-- Tracker UI tables: same shape as football_teams / football_team_contacts, but only teams not in the scraped CSV.
-- Master tables football_teams + football_team_contacts stay the full list.
-- Run in Supabase SQL Editor after football_teams, football_team_contacts, and set_updated_at() exist.
--
-- Exclusion labels must stay in sync with scrapedContactsExcludedCompanies.js (SCRAPED_EXCLUDED_COMPANIES).
-- After this file, run supabase-football-tracker-ui-rebuild-from-master.sql once to copy data and remove excluded teams.
