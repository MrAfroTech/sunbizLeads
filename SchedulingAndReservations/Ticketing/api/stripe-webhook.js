...
module.exports = async (req, res) => {
  ...
  console.log('🔔 Farmer Banks Stripe Webhook received');
...
async function handleCheckoutCompleted(session) {
  ...
  try {
    ...
    const eventName = metadata.event_name || 'Monthly Farm Tour';
    ...
    const eventVenue = metadata.event_venue || 'Here On The Farm';
    const organizerName = metadata.organizer_name || 'Farmer Banks Helfrich';
    ...
    const eventSlug = `${(eventName || 'Monthly Farm Tour').toLowerCase().replace(/\s+/g, '_')}_${(eventDate || '').replace(/-/g, '_')}`;
...
async function generateQRCode(transactionId, metadata) {
  ...
  const baseUrl = process.env.BASE_URL || 'https://farmerbanks.eventbrella.us';
    ...
    for (let i = 0; i < metadata.ticketCount; i++) {
      ...
      const qrData = {
        ticketId: ticketId,
        transactionId: transactionId,
        eventName: metadata.eventName || 'Monthly Farm Tour',
        ...
        eventVenue: metadata.eventVenue || 'Here On The Farm',
        organizerName: metadata.organizerName || 'Farmer Banks Helfrich',
        tier: metadata.tier,
        ...
        checksum: checksum,
        version: '1.0',
        source: 'farmer_banks_farm_tours'
      };
...
async function sendConfirmationEmail(email, name, data) {
  ...
  if (TEST_MODE) {
    ...
    console.log('📨 [EMAIL LOG] Would trigger Klaviyo event:', {
      ...
      subject: `Here On The Farm - ${data.eventName || 'Monthly Farm Tour'}`,
      customerName: name,
    ...
    const eventData = {
      data: {
        ...
        attributes: {
          ...
          properties: {
            event_name: data.eventName || 'Monthly Farm Tour',
            ...
            venue_name: data.eventVenue || 'Here On The Farm',
            ...
            customer_name: name,
            organizer_name: data.organizerName || 'Farmer Banks Helfrich',
            event_slug: data.eventSlug || 'cassava_harvest_2026_01_03',
  ...
  const testPayload = {
    data: {
      ...
      attributes: {
        ...
        properties: {
          eventSlug: "monthly_farm_tour_2026_01_03",
          eventName: "Monthly Farm Tour",
